...
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.Sefaria.config ~= nil and not _G.Sefaria.config.debug then
        return
    end
    ...
    vim.notify(
        string.format("[sefaria.nvim@%s] %s", scope, string.format(str, ...)),
        level,
        { title = "sefaria.nvim" }
    )
end
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h Sefaria.options` to read more."
        )
    end
end
